...
1. **N1 — `scripts/names/N1_build_name_frame.py`** reads `data/raw/panel_final.parquet`, `data/raw/panel_filled_gaps.parquet`, `data/raw/missions_cross_section.parquet`, and `data/raw/bmf_unified_processed.parquet`. `names.panel_scope_values` selects the emitted panel categories (the shipped configuration selects `501C3 CHARITY`); the BMF-only frame remains the full BMF minus the full panel. Existing mission manifests are used to mark contaminated `EIN2`s. It writes isolated panel and BMF-only frames as `panel_name_frame.parquet` and `bmf_only_name_frame.parquet` under `data/interim/names/`.
